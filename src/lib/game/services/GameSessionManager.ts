...
import { getCategoryByIdAsync } from '../content/catalog'
...
export async function createSession(
  ...
  const category = await getCategoryByIdAsync(config.categoryId)
  ...
  return {
    ...
    config,
    players: players.map(p => ({ id: p.id, displayName: p.displayName, color: p.color, icon: (p as any).icon })),
    assignments,

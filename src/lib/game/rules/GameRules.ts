...
export function canAddSpecialRole(
  ...
  if (crewFaction <= traitorFaction) {
    return {
      allowed: false,
      reason: 'Crew muss stärker sein als die Verräter (Crewmate + Detective > Impostor + Accomplice).',
    }
  }
...
export function validateSetup(
  ...
  if (!composition) {
    ...
    const traitorFaction = composition.impostor + composition.accomplice
    if (crewFaction <= traitorFaction) {
      errors.push('Crew-Fraktion muss stärker sein als Verräter-Fraktion (Crewmate + Detective > Impostor + Accomplice).')
  ...
  if (config.includeDetective && config.mode === 'hard') {
    errors.push('Detective existiert nicht im HARD-Modus.')
    disabledReasons.detective = 'Detective nur in NORMAL verfügbar.'
  }

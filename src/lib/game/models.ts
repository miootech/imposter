...
export interface RoleInfo {
  id: RoleId
  /** English name — used in-game (Reveal, Voting, Results) */
  displayName: string
  /** German name — used in Settings, Stats, Group Detail */
  germanName: string
  faction: Faction
...
export const ROLES: Record<RoleId, RoleInfo> = {
  crewmate: {
    ...
    germanName: 'Crewmate',         // App-wide English (v2.4.0) — same as displayName
    faction: 'crew',
    defaultEmoji: '🛡️',
    colorVar: 'crewmate',
  ...
  impostor: {
    ...
    defaultEmoji: '👤',
    colorVar: 'impostor',
    description: 'Kennt andere Impostoren und den Accomplice. NORMAL: Kategorie-Hint.',
  },
  accomplice: {
    id: 'accomplice',
    displayName: 'Accomplice',
    germanName: 'Accomplice',
    faction: 'traitor',
    defaultEmoji: '⭐',
    colorVar: 'accomplice',
...
export interface PlayerStats {
  ...
  /**
   * Faction-aware win counts (added in v2.4.0).
   * Used by the 4-segment WinRateBar (Jester / Crew / Impostor / Losses).
   * Old records loaded from IndexedDB may be missing these — always run them
   * through `normalizePlayerStats()` before reading.
   */
  crewWins: number       // Crewmate + Detective wins
  impostorWins: number   // Impostor + Accomplice wins
  jesterWins: number      // Jester wins (mirrors jesterSuccess)
...
export interface Player {
  id: string
  displayName: string
  /** Player-specific color shown in group cards / detail (hex color) */
  color: string
  /** Optional icon: emoji character OR data:image base64 URL. If set, overrides initials. */
  icon?: string
  points: number
  stats: PlayerStats
}
...
/**
 * Normalize stats loaded from IndexedDB so older records (pre-v2.4.0) get the
 * new faction-aware win fields defaulted to 0 instead of `undefined`.
 *
 * Jester wins are backfilled from `jesterSuccess` when `jesterWins` is missing,
 * since jesterSuccess already counts jester wins and was tracked before v2.4.
 */
export function normalizePlayerStats(stats: PlayerStats | undefined | null): PlayerStats {
  if (!stats) return emptyPlayerStats()
  return {
    ...
    jesterSuccess: stats.jesterSuccess ?? 0,
    crewWins: stats.crewWins ?? 0,
    impostorWins: stats.impostorWins ?? 0,
    jesterWins: stats.jesterWins ?? (stats.jesterSuccess ?? 0),
    roleCount: {
      crewmate: stats.roleCount?.crewmate ?? 0,
      detective: stats.roleCount?.detective ?? 0,
      impostor: stats.roleCount?.impostor ?? 0,
      accomplice: stats.roleCount?.accomplice ?? 0,
      jester: stats.roleCount?.jester ?? 0,
    },
  }
}

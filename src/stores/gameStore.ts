...
import { playMusicTrack, stopMusic } from '../lib/game/services/music'

/**
 * Helper: switch music based on the active game screen.
 * Called whenever gameScreen changes.
 */
function syncMusicToScreen(screen: GameScreen): void {
  switch (screen) {
    case 'reveal':
    case 'reveal-between':
    case 'chaos-reveal':
      playMusicTrack('reveal')
      break
    case 'start-player':
    case 'discussion':
      playMusicTrack('discussion')
      break
    case 'voting':
    case 'vote-pass':
      playMusicTrack('voting')
      break
    case 'voting-results':
    case 'elimination':
      playMusicTrack('results')
      break
    case 'results':
      playMusicTrack('results')
      break
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  setActiveTab: (tab) => { set({ activeTab: tab, gameScreen: 'none' }); syncMusicToScreen('none') },
  openGroupDetail: (groupId) => {
    ...
    syncMusicToScreen('none')
  },

  startSetup: () => set({ gameScreen: 'setup' }),
  cancelSetup: () => { set({ gameScreen: 'none' }); syncMusicToScreen('none') },

  startGame: (config, players) => {
    // createSession is now async (loads custom categories from IndexedDB)
    createSession(config, players).then((session) => {
      set({
        session,
        revealIndex: 0,
        votes: new Map(),
  ...
  startPlay: () => { set({ gameScreen: 'start-player' }); syncMusicToScreen('start-player') },

  startVoting: () => { set({ gameScreen: 'voting', votes: new Map(), currentTally: null }); syncMusicToScreen('voting') },
  ...
  finishVoting: () => {
    ...
    syncMusicToScreen('voting-results')
  },

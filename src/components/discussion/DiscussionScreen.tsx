...
export function DiscussionScreen() {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      if (pausedRef.current) return // Timer paused (app in background)
      setRemaining(prev => {
        if (prev <= 1) {
    ...
    // Listen for app pause/resume events
  ...
  const gradientPreset = usePreferencesStore(s => s.gradientPreset)
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center bg-background px-6 overflow-hidden">
      ...
      {gradientPreset !== 'none' && (
        <GradientTimerBg presetId={gradientPreset} warning={isWarning} round={session.round} />
      )}

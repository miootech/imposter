...
import { useGameStore, proceedAfterElimination, endGame } from '@/stores/gameStore'
              ...
              <AnimatePresence key={c.playerId}>
                {visible && (
                  ...
                  >
                    ...
                    >
                      {player.icon ? <IconRenderer icon={player.icon} size={40} /> : player.displayName.slice(0, 2).toUpperCase()}
                    </div>
          ...
          {showRole && (
            ...
            >
              ...
              >
                ...
                {/* Word is intentionally NOT shown here — revealing it would
                     leak information to other players (e.g. if a crewmate is
                     eliminated first, impostors would learn the word). */}
              </div>
              ...
              <div className="mt-8">
                <GameButton
                  ...
                  onClick={() => {
                    ...
                    playSound('select')
                    if (isGameOver) {
                      // Use endGame to properly sync music to results track
                      const session = useGameStore.getState().session
                      if (session?.winnerFaction) {
                        endGame(session.winnerFaction)
                      } else {

...
import { CATALOG, RANDOM_CATEGORY_ID } from '@/lib/game/content/catalog'
...
export function SetupScreen() {
    ...
    (async () => {
      ...
      if (chosen) {
        setConfig(c => ({
          ...
          categoryId: lastCategoryId ?? RANDOM_CATEGORY_ID,
          mode: lastMode,
        }))
      }
      setLoading(false)
  ...
  const detectiveDisabledReason = detectiveBlockedByMode
    ? 'Detective nur in NORMAL verfügbar.'
        ...
        <Section title="Gruppe">
          <div className="-mx-1 flex gap-3 overflow-x-auto px-1 pb-2 no-scrollbar">
            {groups.map(g => (
              ...
              >
                <div
                  className="relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl text-xl"
                  style={{ backgroundColor: `${g.color}25` }}
                >
                  <IconRenderer icon={g.icon} size={40} />
                </div>
        ...
        {composition && (
          <Section title="Verteilung">
            <div className="grid grid-cols-5 gap-2">
              ...
              <CompositionTile label="Acc." count={composition.accomplice} color="var(--accomplice)" />

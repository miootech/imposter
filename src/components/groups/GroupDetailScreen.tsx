      ...
      >
        <div className="mx-auto max-w-md">
          ...
          <div className="flex items-center gap-4">
            <div
              className="relative flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-3xl text-3xl shadow-sm"
              style={{ backgroundColor: `${group.color}30` }}
            >
              <IconRenderer icon={group.icon} size={64} />
            </div>
              ...
              {group.players.map((player, idx) => (
                <ExpandablePlayerCard
                  ...
                  onIconChange={async (icon) => {
                    // Store the icon in the player's icon field (not color!)
                    await updatePlayer(group.id, player.id, { icon })
                    await refresh()
                  }}
                />
              ))}
